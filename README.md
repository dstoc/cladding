...
- Direct egress from the agent container is [blocked](scripts/jail_agent.sh) except to:
  ...
  - `host.containers.internal` on ports specified in [`config-template/agent_host_ports.lst`](config-template/agent_host_ports.lst).
...
- HTTP(S) from both agent and network sandbox is allowed through Squid on `<name>-proxy`, which only permits domains from:
  - `.cladding/config/agent_domains.lst` (template: [`config-template/agent_domains.lst`](config-template/agent_domains.lst))
  - `.cladding/config/nw_sandbox_domains.lst` (template: [`config-template/nw_sandbox_domains.lst`](config-template/nw_sandbox_domains.lst))
...
* Edit files under `.cladding/config/`:
  ...
  - `.cladding/config/nw_sandbox/` (template dir: [`config-template/nw_sandbox/`](config-template/nw_sandbox/))
  - `.cladding/config/agent_domains.lst` (template: [`config-template/agent_domains.lst`](config-template/agent_domains.lst))
  - `.cladding/config/nw_sandbox_domains.lst` (template: [`config-template/nw_sandbox_domains.lst`](config-template/nw_sandbox_domains.lst))
