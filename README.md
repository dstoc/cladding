...
- `mcp-run` only executes commands allowed by policy:
  - primary: Rego modules in `.cladding/config/sandbox_commands/` (templates under [`config-template/sandbox_commands/`](config-template/sandbox_commands/))
  - transitional fallback: legacy JSON `.cladding/config/sandbox_commands.json` (template: [`config-template/sandbox_commands.json`](config-template/sandbox_commands.json))
